...
function saveGcode() {
    ...
    for (var prop in currentData) {
        if (currentData.hasOwnProperty(prop))
            ++count;
        lines.push(";------" + prop + ": " + currentData[prop]);
        lines.push(";-----------------" + lBr);
        lines.push(lBr);
        lines.push(lBr);
    }
    ...
    var rowExtrusion = currentData.width * currentData.exFeedrate;
    var colExtrusion = currentData.height * currentData.exFeedrate;
    ...
    lines.push("; Move to the first point on the lowest Z axis and set the movement feedrate" + lBr);

    var zHeight = currentData.initHeight;
    for (var layer = 0; layer < currentData.layers; layer++) {
        ...
        for (var row = 1; row <= currentData.rows; row++) {
            ...
            if (extruderSide === "l") { //if the extruder is on the left side, it will have to move to the right side
                ...
                lines.push("G1 X" + leftMargin + " Y" + currPos + " Z" + zHeight + " F" + currentData.frTravel + lBr);
                //lines.push("; Build up pressure" + lBr);
                bup = currentExtrusion + currentData.bupPressExt;
                lines.push("G1 E" + bup + " F" + currentData.frExtruder + lBr);
                lines.push("G92 E" + currentExtrusion + lBr);
                lines.push("G1 F3000" + lBr);
                //lines.push("; To move to the next point while printing" + lBr);
                currentExtrusion += rowExtrusion;
                currPos = topMargin - rowsSpacing * row;
                lines.push("G1 X" + rightMargin + " Y" + currPos + " E" + currentExtrusion + lBr);
                extruderSide = "r";
                //lines.push("; Release pressure to move to the next point without printing" + lBr);
                lines.push("G1 E" + currentExtrusion + " F" + currentData.frExtruder + lBr);
                lines.push("G1 F" + currentData.frTravel + lBr);
                ...
                currPos = topMargin - rowsSpacing * row;
                lines.push("G1 X" + rightMargin + " Y" + currPos + " Z" + zHeight + " F" + currentData.frTravel + lBr);
                //lines.push("; Build up pressure" + lBr);
                bup = currentExtrusion + currentData.bupPressExt;
                lines.push("G1 E" + bup + " F" + currentData.frExtruder + lBr);
                lines.push("G92 E" + currentExtrusion + lBr);
        ...
        while (nextColumn) {
            ...
            if (extruderSide === "b") { //if the extruder is on the left side, it will have to move to the right side
                ...
                lines.push("G1 X" + currPos + " Y" + botMargin + " Z" + zHeight + " F" + currentData.frTravel + lBr);
                //lines.push("; Build up pressure" + lBr);
                bup = currentExtrusion + currentData.bupPressExt;
                lines.push("G1 E" + bup + " F" + currentData.frExtruder + lBr);
                lines.push("G92 E" + currentExtrusion + lBr);
                ...
                currPos = leftMargin + colsSpacing * currentCol;
                lines.push("G1 X" + currPos + " Y" + topMargin + " Z" + zHeight + " F" + currentData.frTravel + lBr);
                //lines.push("; Build up pressure" + lBr);
                bup = currentExtrusion + currentData.bupPressExt;
                lines.push("G1 E" + bup + " F" + currentData.frExtruder + lBr);
                lines.push("G92 E" + currentExtrusion + lBr);
